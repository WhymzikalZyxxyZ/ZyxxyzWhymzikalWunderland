...
const LANGS = {
    ...
    ts:  { label: 'TypeScript', color: '#3178c6', role: 'Typed JS · API · shared types' },
    rs:  { label: 'Rust',       color: '#ce422b', role: 'Systems · WASM · max performance' },
    sw:  { label: 'Swift',      color: '#ff6b35', role: 'iOS · macOS · Apple ecosystem' },
    dt:  { label: 'Dart',       color: '#0175c2', role: 'Flutter · cross-platform UI' },
    cpp: { label: 'C++',        color: '#00599c', role: 'Engine · native performance · FFI' },
};

const METRICS = [
    { key: 'cpu',       label: 'CPU Performance',  scores: { js: 4, go: 9, kt: 7, cs: 8, py: 3, jv: 7, ts: 5,  rs: 10, sw: 9,  dt: 6,  cpp: 10 } },
    { key: 'io',        label: 'I/O Concurrency',  scores: { js: 8, go: 10, kt: 8, cs: 9, py: 6, jv: 7, ts: 8,  rs: 9,  sw: 8,  dt: 8,  cpp: 7  } },
    { key: 'safety',    label: 'Type Safety',       scores: { js: 2, go: 7, kt: 10, cs: 8, py: 4, jv: 7, ts: 9,  rs: 10, sw: 9,  dt: 8,  cpp: 5  } },
    { key: 'ecosystem', label: 'Ecosystem',         scores: { js: 10, go: 6, kt: 7, cs: 8, py: 10, jv: 9, ts: 10, rs: 7,  sw: 6,  dt: 7,  cpp: 9  } },
    { key: 'xplat',     label: 'Cross-platform',   scores: { js: 9, go: 7, kt: 8, cs: 9, py: 9, jv: 9, ts: 9,  rs: 9,  sw: 4,  dt: 10, cpp: 8  } },
    { key: 'learn',     label: 'Ease of Learning',  scores: { js: 8, go: 8, kt: 6, cs: 5, py: 10, jv: 5, ts: 7,  rs: 2,  sw: 6,  dt: 7,  cpp: 1  } },
];
...
const ROWS = [
    { category: 'Core', items: [
        { label: 'Paradigm',
            ...
            ts: 'multi-paradigm', rs: 'multi-paradigm',  sw: 'multi-paradigm',
            dt: 'OOP / functional', cpp: 'multi-paradigm' },
        { label: 'Type system',
            ...
            ts: pill('static','green'),    rs: pill('static','green'),   sw: pill('static','green'),
            dt: pill('static','green'),    cpp: pill('static','green') },
        { label: 'Null safety',
            js: pill('none','red'),         go: pill('zero values','blue'),  kt: pill('enforced','green'),
            cs: pill('opt-in','yellow'),    py: pill('none','red'),          jv: pill('none','red'),
            ts: pill('strict','green'),     rs: pill('Option&lt;T&gt;','green'), sw: pill('optional','green'),
            dt: pill('sound','green'),      cpp: pill('none','red') },
        { label: 'Compilation',
            js: 'JIT / interpreted', go: 'AOT native', kt: 'JVM / KMP native',
            cs: 'JIT CLR / WASM',   py: 'interpreted (CPython)', jv: 'JVM bytecode (JIT)',
            ts: 'transpiled → JS',  rs: 'AOT native',            sw: 'AOT native (LLVM)',
            dt: 'JIT + AOT',        cpp: 'AOT native' },
    ]},
    { category: 'Concurrency', items: [
        { label: 'Model',
            ...
            ts: 'Event loop (Node)', rs: 'async/await + threads', sw: 'async/await + actors',
            dt: 'async/await + isolates', cpp: 'std::thread / co_await' },
        { label: 'Async syntax',
            js: 'async/await', go: 'go func()', kt: 'suspend fun',
            cs: 'async Task',  py: 'async/await', jv: 'CompletableFuture',
            ts: 'async/await', rs: 'async fn',    sw: 'async/await',
            dt: 'async/await', cpp: 'co_await (C++20)' },
        { label: 'CPU parallelism',
            js: pill('none','red'),          go: pill('GOMAXPROCS','green'), kt: pill('JVM threads','blue'),
            cs: pill('ThreadPool','blue'),   py: pill('GIL limited','red'),  jv: pill('native threads','green'),
            ts: pill('none','red'),          rs: pill('native threads','green'), sw: pill('actors','blue'),
            dt: pill('isolates','blue'),     cpp: pill('native threads','green') },
    ]},
    { category: 'Performance', items: [
        { label: 'Startup',
            ...
            ts: pill('fast','green'),   rs: pill('instant','green'),  sw: pill('instant','green'),
            dt: pill('JIT warmup','yellow'), cpp: pill('instant','green') },
        { label: 'Memory',
            js: 'medium', go: pill('low','green'),     kt: pill('high (JVM)','orange'),
            cs: 'medium', py: 'medium',                jv: pill('high (JVM)','orange'),
            ts: 'medium', rs: pill('minimal','green'), sw: pill('low','green'),
            dt: 'medium', cpp: pill('manual','green') },
        { label: 'GC',
            js: 'V8 generational', go: 'tricolor concurrent', kt: 'JVM G1/ZGC',
            cs: '.NET generational', py: 'ref-count + cyclic', jv: 'G1/ZGC/Shenandoah',
            ts: 'V8 generational', rs: 'ownership / RAII', sw: 'ARC',
            dt: 'generational',    cpp: 'manual / RAII' },
    ]},
    { category: 'Ecosystem', items: [
        { label: 'Package mgr',
            js: 'npm', go: 'go mod', kt: 'Gradle', cs: 'NuGet', py: 'pip / Poetry', jv: 'Maven / Gradle',
            ts: 'npm', rs: 'Cargo', sw: 'Swift PM', dt: 'pub.dev', cpp: 'Conan / vcpkg / CMake' },
        { label: 'Test framework',
            js: 'Jest', go: 'testing (stdlib)', kt: 'JUnit / Kotest', cs: 'xUnit', py: 'pytest', jv: 'JUnit 5',
            ts: 'Jest / Vitest', rs: 'cargo test', sw: 'XCTest', dt: 'package:test', cpp: 'Google Test / Catch2' },
        { label: 'Linter',
            js: 'ESLint', go: 'golangci-lint', kt: 'ktlint', cs: 'Roslyn', py: 'Ruff / flake8', jv: 'Checkstyle',
            ts: 'ESLint + tsc', rs: 'clippy', sw: 'SwiftLint', dt: 'dart analyze', cpp: 'clang-tidy' },
    ]},
];
...
const MATRIX = [
    ...
    { problem: 'Chess AI — search speed',         best: 'rs',  runner: 'cpp', avoid: ['py','js'] },
    { problem: 'Typed JS — strict, same team',    best: 'ts',  runner: 'js',  avoid: [] },
    { problem: 'Memory-safe systems code',        best: 'rs',  runner: 'cpp', avoid: ['py','js'] },
    { problem: 'iOS / macOS native app',          best: 'sw',  runner: null,  avoid: ['kt','py'] },
    { problem: 'Cross-platform mobile (Flutter)', best: 'dt',  runner: 'cs',  avoid: ['rs','cpp'] },
    { problem: 'WASM / browser native perf',      best: 'rs',  runner: 'cpp', avoid: ['py','jv'] },
    { problem: 'Systems / embedded programming',  best: 'cpp', runner: 'rs',  avoid: ['py','js'] },
];
...
const SUMMARY = [
    ...
    { key: 'ts',  text: 'The <em>safety net</em>. JavaScript that refuses to lie — the same runtime, honest about everything it carries.' },
    { key: 'rs',  text: 'The <em>enforcer</em>. No runtime. No GC. No data races. The compiler holds the line so nothing else has to.' },
    { key: 'sw',  text: 'The <em>Apple insider</em>. SwiftUI speaks the platform\'s native language. Everywhere else is translation.' },
    { key: 'dt',  text: 'The <em>shapeshifter</em>. One codebase, every screen. Flutter renders identically on phone, desktop, and web.' },
    { key: 'cpp', text: 'The <em>foundation stone</em>. Everything eventually runs on C++. It doesn\'t abstract — it owns.' },
];
...
const PROFILES = {
    ...
    ts: {
        tagline: 'JavaScript, finally willing to admit what it is.',
        desc: 'TypeScript is not a different language — it compiles to the same JavaScript that runs everywhere. What it adds is honesty. The type system surfaces the contracts that always existed but were never enforced. A team that moves from JS to TypeScript does not rewrite anything; they make the invisible visible.',
        strengths: [
            { text: 'Structural typing catches interface mismatches at build, not runtime',
              detail: 'TypeScript checks shape, not identity. If a function expects { game: string; value: number }, any object with those fields satisfies it — regardless of what class it came from. This removes an entire category of "works in dev, breaks in prod" bugs.' },
            { text: 'Incremental adoption — rename .js to .ts and add types gradually',
              detail: 'TypeScript\'s strict mode can be enabled file by file. A codebase can migrate over weeks or months without a big-bang rewrite. allowJs lets JS and TS files coexist in the same project until every file is converted.' },
            { text: 'Same npm ecosystem — every JS package is immediately available',
              detail: 'npm\'s 2.5 million packages are all accessible. Most popular libraries publish @types/* declarations. Even without them, declare module can paper over an untyped package until types are added upstream.' },
            { text: 'IDE autocomplete and refactoring become accurate, not guesswork',
              detail: 'VS Code and JetBrains use the TypeScript language server for go-to-definition, rename-symbol, find-all-references, and extract-function. In plain JS, these operations guess. In TypeScript, they are provably correct across the entire codebase.' },
            { text: 'Generics, discriminated unions, and conditional types model complex domains',
              detail: 'type Result&lt;T&gt; = { ok: true; value: T } | { ok: false; error: string } is a discriminated union — the compiler exhausts every branch. This pattern eliminates the "check if it has .error before reading .value" defensive boilerplate that JS forces everywhere.' },
        ],
        limits: [
            { text: 'Adds a build step — plain JS has no compilation phase',
              detail: 'Every TypeScript project needs tsc, ts-node, tsx, or a bundler (Vite, esbuild) to produce runnable code. This is trivial to set up but means a TypeScript hello-world has more moving parts than a JS equivalent.' },
            { text: 'Type errors are compile-time only — the runtime is still plain JavaScript',
              detail: 'TypeScript guarantees nothing at runtime. A value arriving from an API, localStorage, or JSON.parse is typed as unknown or any by default and has no runtime validation. Libraries like Zod are required to bridge the gap between declared types and actual data.' },
            { text: 'Complex generics can produce unreadable error messages',
              detail: 'Deeply nested conditional types — common in framework internals — generate error messages dozens of lines long. A type error in a generic utility function can produce a stack trace that requires expert knowledge of TypeScript\'s type inference algorithm to interpret.' },
            { text: 'tsconfig sprawl — projects need careful configuration to avoid footguns',
              detail: 'strict mode alone sets 9 compiler flags. Add module, moduleResolution, target, lib, paths, and incremental — all interacting. A misconfigured tsconfig silently disables checks that the team assumed were active. The defaults before strict mode are permissive to the point of being misleading.' },
            { text: 'Still single-threaded — TypeScript does not fix JS\'s CPU parallelism problem',
              detail: 'TypeScript compiles to JavaScript. The V8 event loop and its single-threaded CPU execution model are unchanged. Web Workers help, but the communication overhead and serialization cost remain the same as in plain JavaScript.' },
        ],
    },
    rs: {
        tagline: 'The one that makes the compiler do the dangerous work.',
        desc: 'Rust has no garbage collector. No runtime exceptions. No null dereferences. No data races — not as a convention but as a compile-time guarantee. The borrow checker is the mechanism: it proves that every value has exactly one owner at a time and that references never outlive what they point to. The cost is a steep learning curve. The payoff is code that is simultaneously as fast as C++ and as memory-safe as Go.',
        strengths: [
            { text: 'Borrow checker eliminates use-after-free, double-free, and data races at compile time',
              detail: 'The compiler tracks ownership and lifetimes statically. A reference cannot outlive the value it points to — this is checked at every call site. This eliminates an entire class of security vulnerabilities (CVE categories: heap corruption, UAF, race conditions) without any runtime overhead.' },
            { text: 'Zero-cost abstractions — iterators, generics, and closures compile to the same code as hand-written loops',
              detail: 'Rust\'s iterator chain (board.iter().flatten().filter(|&&p| p > 0)) compiles to the same machine code as the equivalent for loop. The abstraction has no runtime cost. This is the property C++ has always claimed and Rust enforces via the type system.' },
            { text: 'Cargo is the best build + dependency tool in this stack, by a significant margin',
              detail: 'cargo build, cargo test, cargo bench, cargo doc, cargo publish — one tool for every lifecycle phase. Dependency resolution is deterministic via Cargo.lock. Publishing to crates.io takes one command. No Gradle configuration files, no Maven POM, no CMakeLists required.' },
            { text: 'Compiles to native binary and to WASM with the same source code',
              detail: 'cargo build --target wasm32-unknown-unknown produces a WASM module. The same chess engine runs natively in the desktop app and in the browser via WASM without maintaining two codebases. wasm-bindgen generates the JS glue automatically.' },
            { text: 'Fearless concurrency — the type system prevents data races across threads',
              detail: 'Send and Sync are marker traits the compiler checks automatically. If a type is not thread-safe, it cannot be sent across a thread boundary — the code does not compile. This rules out the most common category of concurrency bugs before they reach a test environment.' },
        ],
        limits: [
            { text: 'Steepest learning curve in this stack — borrow checker fights beginners',
              detail: 'The borrow checker rejects valid-feeling code that would be fine in any other language. Writing a function that takes two references to the same struct field fails until lifetimes are explicit. Most developers report 2–4 weeks of frustration before the mental model clicks.' },
            { text: 'Compile times are the slowest in this stack for large codebases',
              detail: 'Rust\'s monomorphisation of generics generates a separate compiled function for each concrete type. A large project with many generic data structures can take 5–15 minutes for a clean build. Incremental builds are faster but still slower than Go, TypeScript, or C#.' },
            { text: 'Async Rust requires choosing an executor (Tokio, async-std) — no stdlib default',
              detail: 'The standard library defines the async/await syntax and the Future trait but provides no runtime. Every async Rust project must choose and add Tokio or another executor as a dependency. Mixing executors in one binary causes subtle incompatibilities.' },
            { text: 'Fewer available libraries than Go, Python, or TypeScript in most application domains',
              detail: 'crates.io has ~150k packages — significantly fewer than npm (2.5M) or PyPI (500k). Systems libraries are mature. Web frameworks (Axum, Actix), game engines, and crypto are solid. ML, data science, and GUI toolkits are thin compared to Python and JS equivalents.' },
            { text: 'Lifetimes in structs add verbose annotations that can obscure intent',
              detail: 'A struct that holds a reference must annotate the lifetime of that reference explicitly: struct Board&lt;\'a&gt; { data: &amp;\'a [[i8;8];8] }. For complex data structures with multiple references, lifetime parameters multiply. The practical solution is to use owned types (Vec, String) and clone when necessary — which has its own cost.' },
        ],
    },
    sw: {
        tagline: 'Designed for one platform. Unrivalled on it.',
        desc: 'Swift is Apple\'s language for Apple\'s platforms. SwiftUI is the only framework with first-class access to every iOS and macOS component — HapticFeedback, Face ID, Core Data, HealthKit, ARKit. No other language in this stack can reach these APIs without a bridge. If the target is an iPhone or a Mac, Swift is not a preference — it is the ground truth.',
        strengths: [
            { text: 'SwiftUI: declarative, previewed, and animated — first-class on every Apple platform',
              detail: 'SwiftUI renders natively on iOS, macOS, watchOS, tvOS, and visionOS from a single View body. Xcode previews show the rendered UI in real time as code is typed. Animations are implicit — adding .animation(.spring) to a state-driven component requires no animation code.' },
            { text: 'ARC memory management: deterministic, no GC pauses',
              detail: 'Automatic Reference Counting is resolved at compile time. Object deallocation is deterministic — it happens exactly when the last strong reference drops, not at a GC collection point. No stop-the-world pauses. No memory pressure callbacks. Predictable frame timing on 120 Hz ProMotion displays.' },
            { text: 'Value types by default — structs, enums, and tuples copied on assignment',
              detail: 'Swift structs are value types. Passing a ChessGame struct to a function does not share state — it copies. Mutations are local unless explicitly marked inout. This eliminates the entire class of "someone mutated the shared state" bugs that plague reference-type-heavy languages.' },
            { text: 'Optionals are typed and compiler-enforced — Int? cannot be used as Int',
              detail: 'Optional&lt;T&gt; is a first-class type with compiler enforcement. Unwrapping nil causes a compile warning with if let or guard let, and a controlled runtime trap (not an unhandled exception) if force-unwrapped. There is no silent null propagation.' },
            { text: 'Concurrency model with actors prevents data races at the language level',
              detail: 'Swift actors serialize access to their mutable state automatically. Calling an actor\'s method from outside requires await, which the compiler enforces. @MainActor marks types that must execute on the main thread — the compiler rejects cross-actor state access without await.' },
        ],
        limits: [
            { text: 'Apple platform only — no first-class Linux, Windows, or Android support',
              detail: 'Swift is open source and runs on Linux (used by some server-side Swift projects), but the SwiftUI framework is Apple-only. Building a Windows or Android app in Swift requires maintaining platform-specific UI code that has no parity with the iOS/macOS tier. This is not a realistic cross-platform strategy.' },
            { text: 'Xcode is mandatory — development without macOS is effectively impossible',
              detail: 'Signing, provisioning profiles, Simulator, Instruments, and the app distribution pipeline all require Xcode on macOS. Linux Swift supports server-side development but cannot build iOS apps. CI for Swift must run on macOS agents, which cost more than Linux.' },
            { text: 'Smaller ecosystem than JS, Python, or Go outside the Apple domain',
              detail: 'Swift Package Manager has a growing registry, but outside iOS/macOS development the package count and community size are a fraction of npm or crates.io. Data science, ML (outside Core ML), and web tooling libraries are sparse compared to Python.' },
            { text: 'ABI stability arrived late — pre-Swift 5 frameworks cannot be mixed with Swift 5 binaries',
              detail: 'Swift\'s ABI was not stable until Swift 5.0 (2019). Code compiled before that cannot be linked with modern Swift modules without recompilation. For shipping framework SDKs to third parties, this history means older clients may require special handling.' },
            { text: 'Compile times for large SwiftUI projects can be slow',
              detail: 'SwiftUI view bodies with complex type inference (deep ViewBuilder chains, conditional views, environment modifiers) can take seconds per file to compile. The type-checker is the bottleneck. Splitting large views into smaller subcomponents is a necessary optimization strategy, not a preference.' },
        ],
    },
    dt: {
        tagline: 'One canvas. Every device.',
        desc: 'Flutter draws every pixel itself — it does not wrap native controls. The same widget tree renders identically on a Pixel phone, an iPhone, a macOS window, a Windows desktop, and a browser. Dart is the language that drives it: fast JIT for development, optimized AOT for release. The trade-off is a custom renderer that feels slightly foreign on each platform. The gain is true pixel-perfect consistency across all of them.',
        strengths: [
            { text: 'Flutter renders identically on iOS, Android, macOS, Windows, Linux, and Web',
              detail: 'Flutter\'s Skia/Impeller renderer draws every widget directly to a GPU canvas. There is no native control translation layer. A BoxDecoration or AnimatedContainer looks and behaves the same on every platform. This is a meaningful advantage for teams that cannot afford per-platform UI work.' },
            { text: 'Hot reload: see UI changes reflected in under a second without losing app state',
              detail: 'Flutter\'s hot reload injects updated Dart code into the running VM without a full restart. State is preserved. Changing a color, a layout, or an animation and pressing save shows the result in the emulator within 500 ms. This dramatically compresses the design-to-implementation feedback loop.' },
            { text: 'Sound null safety — the compiler proves every variable is initialized before use',
              detail: 'Dart\'s sound null safety (activated by default since Dart 2.12) divides the type system into nullable (T?) and non-nullable (T). The compiler proves at compile time that a non-nullable variable cannot hold null — not through inference, but through control flow analysis. There is no null safety annotation you can forget.' },
            { text: 'Dart isolates provide true CPU parallelism without shared memory',
              detail: 'Dart isolates are like processes: each has its own heap and memory. Communication is via message passing (SendPort/ReceivePort). There is no shared mutable state between isolates, so no data races are possible. The chess AI runs in a separate isolate, keeping the UI at 60 fps regardless of search depth.' },
            { text: 'pub.dev ecosystem has high-quality Flutter plugins for most mobile APIs',
              detail: 'pub.dev hosts packages for camera, Bluetooth, local notifications, in-app purchases, Google Maps, Firebase, and platform-specific channels for every major mobile API. Package quality is generally high — Flutter\'s package scoring system (pub.dev likes/points) surfaces maintained, documented packages.' },
        ],
        limits: [
            { text: 'Custom renderer — does not use native platform widgets by default',
              detail: 'A Flutter TextField is not a UITextField or an Android EditText. It is Flutter\'s own widget that approximates the platform\'s look and feel. Platform-specific behaviors (context menus, selection handles, keyboard types) may diverge from native expectations, especially on desktop platforms.' },
            { text: 'Web target is functional but app size and SEO are real constraints',
              detail: 'Flutter Web compiles to WASM or JS-heavy canvaskit output. The initial load includes the entire Flutter engine (~2 MB compressed) plus the app. SEO requires a separate server-side rendering strategy since the HTML shell contains no meaningful content for crawlers.' },
            { text: 'Smaller community than React Native or native iOS/Android',
              detail: 'Flutter\'s community is large and growing, but it is significantly smaller than the React Native ecosystem. For unusual native platform integrations, you may find yourself writing a platform channel plugin from scratch rather than installing an existing package.' },
            { text: 'Dart is Flutter-specific — it is not a general-purpose ecosystem language',
              detail: 'Outside Flutter, Dart is used for some CLI tools and server-side packages, but the community and package ecosystem are small. A developer who learns Dart for Flutter has limited transfer value to non-Flutter work — unlike TypeScript (runs everywhere) or Kotlin (Android + server).' },
            { text: 'AOT binary size is larger than native equivalents for simple apps',
              detail: 'A minimal Flutter app includes the full Flutter engine regardless of how many widgets are used. On Android, a simple Flutter APK is 6–8 MB where a comparable native app might be 1–2 MB. Tree-shaking reduces this but cannot eliminate the engine baseline.' },
        ],
    },
    jv: {
    ...
    cpp: {
        tagline: 'What everything else compiles to, eventually.',
        desc: 'C++ is the language that owns the substrate. The V8 engine is written in C++. The JVM is written in C++. CPython is written in C. The .NET CLR is written in C++. Every virtual machine, every runtime, every hardware abstraction layer that the other languages in this stack depend on runs on C++. Using it directly means writing at the level where performance is no longer negotiated — it is commanded.',
        strengths: [
            { text: 'Maximum CPU performance — zero-overhead abstractions, manual memory control',
              detail: 'C++ templates generate specialized machine code for each concrete type — no boxing, no virtual dispatch unless you request it. Manual memory management via new/delete or RAII means allocation is exactly as expensive as you decide it is. Stock-exchange matching engines, game physics engines, and audio DSPs are written in C++ for this reason.' },
            { text: 'RAII: resources are automatically released when they go out of scope',
              detail: 'std::unique_ptr, std::lock_guard, and std::ofstream release their resources in their destructors. When a scope exits (normally or via exception), the stack unwinds and every destructor fires in reverse order. This is deterministic cleanup without a GC and without manual cleanup code at every exit path.' },
            { text: 'Interoperability with every other language via C ABI',
              detail: 'Any language with a foreign function interface (Python\'s ctypes, Rust\'s extern "C", Go\'s cgo, JNI for Java/Kotlin) can call a C++ function exposed through a C-compatible header. This makes C++ the universal bridge — a chess engine written in C++ can be consumed by every other language in this stack.' },
            { text: 'Template metaprogramming enables zero-cost compile-time computation',
              detail: 'constexpr functions and template specializations execute at compile time. A chess piece-square table can be fully computed at build time with no runtime cost. std::array bounds are checked at compile time. These techniques are used in game engines and real-time audio to eliminate entire categories of runtime overhead.' },
            { text: 'Mature toolchain: CMake, Conan, vcpkg, clang-tidy, AddressSanitizer, Valgrind',
              detail: 'AddressSanitizer detects heap corruption, use-after-free, and buffer overflows at runtime with minimal overhead — more precise than Valgrind. clang-tidy enforces coding guidelines and catches common bugs statically. This toolchain is battle-hardened on codebases of tens of millions of lines.' },
        ],
        limits: [
            { text: 'Steepest learning curve in the stack — undefined behavior is invisible and lethal',
              detail: 'Accessing memory out of bounds, using a moved-from object, or reading an uninitialized variable is undefined behavior in C++. The program does not crash reliably — it may run correctly in debug builds and corrupt data silently in release builds. Understanding what is undefined behavior and avoiding it requires years of experience.' },
            { text: 'Manual memory management — use-after-free, double-free, and leaks without discipline',
              detail: 'Without smart pointers and RAII, memory must be manually allocated and freed. Missing a delete causes a leak. Deleting twice causes corruption. Modern C++ (std::unique_ptr, std::shared_ptr) makes this safe, but legacy codebases and C-style code are ubiquitous and often mixed into new projects.' },
            { text: 'No standard build system — CMake, Meson, Bazel, and autotools all coexist',
              detail: 'There is no language-standard build tool analogous to Cargo or go build. CMake is the de facto standard but its syntax is arcane and its errors are cryptic. Integrating a new dependency requires understanding the build system, the package manager (if any), and potentially writing CMake find modules.' },
            { text: 'Compilation errors on templates are notoriously long and hard to read',
              detail: 'A type mismatch in a template argument chain can produce error messages spanning hundreds of lines, naming internal template instantiations rather than the actual error site. Concepts (C++20) improve this significantly for new code, but generic library code written before C++20 still produces opaque diagnostics.' },
            { text: 'No memory safety guarantees — CVE categories that Rust eliminates statically',
              detail: 'The majority of critical CVEs in widely deployed C++ codebases (browsers, kernels, media players) are memory safety violations: buffer overflows, use-after-free, type confusion. These are fundamentally architectural risks in C++ that require discipline, tooling, and fuzzing to manage — not eliminate.' },
        ],
    },
};
...
const USECASES = {
    ...
    ts:  ['TypeScript chess engine (strict interfaces)', 'Express REST API with minimal JWT', 'Shared types across client + server', 'tsc + ESLint CI pipeline', 'Adjacent: OpenAPI schema generation'],
    rs:  ['zyxxyz_chess static lib (stack Board type)', 'Axum REST API (no GC pauses)', 'egui desktop chess client', 'Adjacent: WASM chess engine for browser', 'Adjacent: zero-copy CLI tooling'],
    sw:  ['SwiftUI chess app (macOS native)', '@MainActor ChessViewModel isolation', 'Task.detached AI computation', 'Adjacent: iOS / visionOS targets', 'SwiftPM multi-target module layout'],
    dt:  ['Flutter chess app (GridView board)', 'Shared chess_engine Dart package', 'Future.microtask AI dispatch', 'Adjacent: Flutter Web + Desktop targets', 'Sound null safety throughout engine'],
    cpp: ['chess_engine static lib (C++17)', 'std::array board (stack-allocated)', 'Optional: SDL2 chess desktop client', 'Adjacent: engine FFI for Python / Rust', 'CMake multi-target build system'],
};
