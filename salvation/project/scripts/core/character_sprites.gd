...
##
## Every build_*() result is cached (see _cached below) — a given
## call's actual pixel output never varies for the same arguments (a
## Sinner's shape never changes between spawns, Pride's palette is always
## the exact same Color), so without caching, every single enemy/character
## spawn was redundantly re-running the same PixelCanvas fill/outline/bake
## work from scratch. Tinting (e.g. an enemy's red attack-slash) is applied
## via Sprite2D.modulate at the call site, never baked into the texture
## itself, so sharing one Texture2D across every instance of "the same
## sprite" is always safe.

## Keyed by an arbitrary caller-chosen string (function name, optionally
## plus a stringified argument for parameterized builders like
## build_sin_boss's base_color) — not automatic, each build_*() below
## picks its own key.
...
static func build_paladin() -> Texture2D:
	return _cached("paladin", func() -> Texture2D:
...
static func build_cleric() -> Texture2D:
	return _cached("cleric", func() -> Texture2D:
		var canvas := PixelCanvas.new(18, 20)
...
static func build_monk() -> Texture2D:
	return _cached("monk", func() -> Texture2D:
...
static func build_exorcist() -> Texture2D:
	return _cached("exorcist", func() -> Texture2D:
...
static func build_prophet() -> Texture2D:
	return _cached("prophet", func() -> Texture2D:
...
static func build_projectile_bolt() -> Texture2D:
	return _cached("projectile_bolt", func() -> Texture2D:
...
## Robe hue is parametrized (see Sinner.ROBE_COLORS) rather than fixed —
## eyes and outline stay constant across every variant, same "hold some
## keys constant, derive the rest from one hue" approach _sin_palette uses
## for the ten trial bosses, just applied one tier down so a room full of
## Sinners doesn't read as one enemy copy-pasted repeatedly.
static func build_sinner(robe_color: Color) -> Texture2D:
	return _cached("sinner:%s" % robe_color, func() -> Texture2D:
		...
		var palette := {
			"r": robe_color,
			"R": robe_color.darkened(0.35),
			"H": robe_color.lightened(0.2),
...
static func build_sin_boss(base_color: Color) -> Texture2D:
	return _cached("sin_boss:%s" % base_color, func() -> Texture2D:
...
static func build_envy_boss(base_color: Color) -> Texture2D:
	return _cached("envy_boss:%s" % base_color, func() -> Texture2D:
...
static func build_wrath_boss(base_color: Color) -> Texture2D:
	return _cached("wrath_boss:%s" % base_color, func() -> Texture2D:
...
static func build_gluttony_boss(base_color: Color) -> Texture2D:
	return _cached("gluttony_boss:%s" % base_color, func() -> Texture2D:
...
static func build_greed_boss(base_color: Color) -> Texture2D:
	return _cached("greed_boss:%s" % base_color, func() -> Texture2D:
		var canvas := PixelCanvas.new(26, 22)
...
static func build_sloth_boss(base_color: Color) -> Texture2D:
	return _cached("sloth_boss:%s" % base_color, func() -> Texture2D:
...
static func build_lust_boss(base_color: Color) -> Texture2D:
	return _cached("lust_boss:%s" % base_color, func() -> Texture2D:
...
static func build_despair_boss(base_color: Color) -> Texture2D:
	return _cached("despair_boss:%s" % base_color, func() -> Texture2D:
...
static func build_doubt_boss(base_color: Color) -> Texture2D:
	return _cached("doubt_boss:%s" % base_color, func() -> Texture2D:
...
## A concentric radial glow — MagicBurst's texture, for whichever Magic
## abilities don't read as a weapon swing (a heal, a smite from above, a
## closing verse). Baked white/neutral, same as build_slash, so every
## caller tints it via Sprite2D.modulate instead of this baking in one
## fixed color — one spell reads as gold, another as green, from the same texture.
static func build_burst() -> Texture2D:
	return _cached("burst", func() -> Texture2D:
		var canvas := PixelCanvas.new(20, 20)
		canvas.fill_ellipse(10, 10, 9.5, 9.5, "o")
		canvas.fill_ellipse(10, 10, 7, 7, "m")
		canvas.fill_ellipse(10, 10, 4, 4, "c")
		canvas.fill_ellipse(10, 10, 1.6, 1.6, "h")

		var palette := {
			"o": Color(1.0, 1.0, 1.0, 0.18),
			"m": Color(1.0, 1.0, 1.0, 0.4),
			"c": Color(1.0, 1.0, 1.0, 0.75),
...
## attacker's facing direction, and tint it via Sprite2D.modulate (e.g. an
## enemy's red slash) rather than baking any color into the texture itself.
static func build_slash() -> Texture2D:
	return _cached("slash", func() -> Texture2D:
		var canvas := PixelCanvas.new(20, 20)
...
static func build_door() -> Texture2D:
	return _cached("door", func() -> Texture2D:
